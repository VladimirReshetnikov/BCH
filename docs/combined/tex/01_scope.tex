...
converges absolutely; its sum $Z$ satisfies $e^Z=e^Xe^Y$ and is the only
element of norm less than $\log2$ with that property; every $Z_n(X,Y)$, and
therefore $Z$ itself, lies in every closed Lie subalgebra of $\cA$ that
contains $X$ and $Y$; and
\[
...
logarithm, and neither is claimed outside its stated setting.
