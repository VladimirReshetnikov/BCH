...
is $B_n^+/n!$. Analytically, for $X,Y$ in a Banach algebra with $\norm X+\norm Y$
sufficiently small, the function $t\mapsto Z(X,tY)$ is differentiable at
$t=0$ with
\begin{equation}\label{eq:linearYderiv}
 \frac{\dd}{\dd t}\Big|_{t=0}Z(X,tY)=\beta(\ad_X)Y
 =\sum_{n\geq0}\frac{B_n^+}{n!}\ad_X^nY.
\end{equation}
\end{corollary}
\begin{proof}
...
$\Gamma$ of positively oriented contours whose interior contains $\spec(A)$
and no point of $2\pi\ii\Z\setminus\{0\}$, so that $\Gamma$ winds zero
times about every pole of $\beta$; it is not necessarily the Taylor series
at zero.
...
The symmetrization
\[
 D^m\beta(A)[E_1,\ldots,E_m]=\sum_{\pi\in S_m}T_m(A)\bigl[E_{\pi(1)},\ldots,E_{\pi(m)}\bigr]
\]
is the $m$th Fr\'echet derivative of the holomorphic matrix function
$\beta$, but the unsymmetrized form is what the recursion needs.
...
\end{theorem}
\begin{proof}
Let $\Omega$ be the closed exterior of $\Gamma$, that is, the complement of
the interior of $\Gamma$. The function $z\mapsto\norm{R(z)}$ is continuous
on $\Omega$ (which contains no point of $\spec(A)$) and tends to $0$ as
$|z|\to\infty$, so $\kappa_\Gamma:=\sup_{z\in\Omega}\norm{R(z)}<\infty$. For
an operator $E$ with $\norm E\,\kappa_\Gamma<1$, the resolvent of $A+E$
exists at every $z\in\Omega$ and is given by the Neumann series
\[
 (zI-A-E)^{-1}=\bigl(R(z)^{-1}(I-R(z)E)\bigr)^{-1}
 =\sum_{k\geq0}\bigl(R(z)E\bigr)^kR(z),
\]
converging uniformly on $\Omega$, and in particular on $\Gamma$. Hence
$\spec(A+E)$ lies in the interior of $\Gamma$, and by the holomorphic
functional calculus
\[
...
$Z(\eps)$; for $|\eps|$ small the condition $\norm{E(\eps)}\kappa_\Gamma<1$
holds. Substituting into
