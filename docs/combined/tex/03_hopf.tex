...
$Y$; and because $\Delta$ raises neither total degree nor lowers it, it
...
\end{lemma}
\begin{proof}
Write $\Delta v=\sum v_{(1)}\otimes v_{(2)}$ in the finite notation of
\eqref{eq:unshuffle} and put $K(v)=\sum v_{(1)}S(v_{(2)})$. For a letter $a$
...
\end{proof}
Thus $T(V)$, and continuously its completion, carries the asserted Hopf
...
$[X,Y]$, and consider the quotient Lie algebra $\cL/J$. Its two generators
commute, so every bracket of length at least two of elements of $\cL/J$
vanishes and $\cL/J$ is abelian. Alternatively and more concretely: a
right-nested bracket $R(w)$ with $|w|\geq2$ has innermost bracket
$[a_{n-1},a_n]$, which is either zero or $\pm[X,Y]$, and an iterated bracket
whose innermost factor lies in the ideal $J$ lies in $J$; since each $Z_n$
with $n\geq2$ is a combination of such brackets by \eqref{eq:dsw}, it lies
in $J$.
\end{proof}
