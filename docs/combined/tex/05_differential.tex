...
In the formal setting, where $X,Y$ have positive degree and $s$ is a formal
or scalar parameter, and for all $X,Y$ in a Banach algebra and all scalars
$s$,
...
difference $G$ of two solutions satisfies
$\norm{G(s)}\leq2\norm X\int_0^s\norm{G}$, hence vanishes by Gr\"onwall's
inequality. The norm bound follows from $\norm{\ad_X^nY}\leq(2\norm X)^n\norm Y$.
...
Conjugation $A\mapsto e^XAe^{-X}$ is an algebra automorphism, continuous in
the Banach case and degree-preserving in the formal case, so it commutes with
the exponential series. Therefore
\begin{align}
