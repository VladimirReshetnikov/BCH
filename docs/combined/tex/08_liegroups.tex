...
with $X\in\g$ is $X^L(g)=gX$. Regarding these as vector fields on the open
...
$\alpha=\theta(\partial_sg)$ and $\beta=\theta(\partial_tg)$ satisfy the
...
$g^*\theta=\alpha\,\dd s+\beta\,\dd t$. Pullback commutes with $\dd$ and
with the bracket of forms, so \eqref{eq:MC} gives
$\dd(g^*\theta)=-[\alpha,\beta]\,\dd s\wedge\dd t$, while directly
$\dd(\alpha\,\dd s+\beta\,\dd t)=(\partial_s\beta-\partial_t\alpha)\,\dd s\wedge\dd t$.
Comparing coefficients gives \eqref{eq:mixedvariation}. (In a matrix group:
$\partial_s(g^{-1}\partial_tg)=-\alpha\beta+g^{-1}\partial_s\partial_tg$ and
$\partial_t(g^{-1}\partial_sg)=-\beta\alpha+g^{-1}\partial_t\partial_sg$;
subtracting gives $-[\alpha,\beta]$.)
\end{proof}
...
$\beta(s,t)=\theta(\partial_tg)$; then $\beta(0,t)=0$ because $g(0,t)=e$
for all $t$. By \eqref{eq:mixedvariation},
$\partial_s\beta=\partial_t\alpha-[\alpha,\beta]=H-[X+tH,\beta]$. At $t=0$
this is the linear equation $\partial_s\beta=H-\ad_X\beta$ with
$\beta(0)=0$, whose unique solution is
$\beta(s)=\int_0^se^{-(s-u)\ad_X}H\,\dd u$ (check by differentiation:
$\partial_s\beta=H-\ad_X\beta$). At $s=1$, substituting $v=1-u$,
$\beta(1,0)=\int_0^1e^{-v\ad_X}H\,\dd v=\phi(\ad_X)H$ by the termwise
integration of \cref{thm:dexp}. Since $\beta(1,0)=\theta(\partial_tg(1,0))
...
\[
 F(\exp X\exp Y)=F\bigl(\exp\BCH(X,Y)\bigr)=\exp_H\bigl(\BCH(fX,fY)\bigr)
