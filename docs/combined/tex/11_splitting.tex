...
\begin{equation}\label{eq:S2all}
 ...
 =e^{t(X+Y)}+O(n^{-2}),
\end{equation}
uniformly for $t$ in compact sets. Explicitly, for $n>6|t|a/\log2$ one has
$S_2(t/n)^n=\exp\bigl(t(X+Y)+E_n\bigr)$ with
$\norm{E_n}\leq c_2|t|^3a^3/n^2$, where $c_2=c_0+28c_0'$ with the constants
of \cref{lem:tail}, and hence, when $\norm I=1$,
\begin{equation}\label{eq:S2error}
 \bigl\|S_2(t/n)^n-e^{t(X+Y)}\bigr\|
 \leq e^{|t|a+c_2|t|^3a^3/n^2}\,\frac{c_2|t|^3a^3}{n^2}.
\end{equation}
\end{proposition}
\begin{proof}
...
The explicit constant is obtained without the multifactor expansion, by two
applications of the two-factor theorem. Put $A=tX/(2n)$, $B=tY/n$,
$\sigma=\norm A+\norm B\leq|t|a/n<\tfrac16\log2$, $W=\BCH(A,B)$ and
$L=\BCH(W,A)$, so that $e^L=e^We^A=e^Ae^Be^A=S_2(t/n)$. Since
$\sigma\leq\tfrac14$, the Mercator bound gives
$\norm W\leq-\log(2-e^{\sigma})\leq2\sigma$ (using $e^\sigma-1\leq\sigma+\sigma^2$
and $-\log(1-u)\leq u/(1-u)$), so $\norm W+\norm A\leq3\sigma<\tfrac12\log2$.
Write $W=A+B+V$ with $V=Z_2(A,B)+T$ the part of degree $\geq2$ and $T$ the
part of degree $\geq3$, and $L=W+A+Z_2(W,A)+T'$ with $T'$ the part of degree
$\geq3$ of $\BCH(W,A)$. Then $Z_2(W,A)=\tfrac12[A+B,A]+\tfrac12[V,A]$ and
$Z_2(A,B)+\tfrac12[A+B,A]=0$, so $L=2A+B+T+\tfrac12[V,A]+T'$, and
\cref{lem:tail} gives $\norm T\leq c_0'\sigma^3$, $\norm V\leq c_0\sigma^2$,
$\norm{T'}\leq c_0'(3\sigma)^3$; hence
$\norm{L-(2A+B)}\leq(c_0+28c_0')\sigma^3=c_2\sigma^3$. Raising $e^L$ to the
$n$th power gives $S_2(t/n)^n=\exp(t(X+Y)+E_n)$ with $E_n=n(L-2A-B)$,
$\norm{E_n}\leq nc_2(|t|a/n)^3=c_2|t|^3a^3/n^2$, and \eqref{eq:expdifference}
yields \eqref{eq:S2error}.
\end{proof}
