...
\begin{aligned}
 e^{-X}\dd e^X:=(\exp^*\theta)_X&=\phi(\ad_X)\,\dd X
...
\begin{gathered}
 ...
 X^{i_1}\cdots X^{i_n}\prod_{t=1}^{n}f_{i_tc_{n-t}}{}^{c_{n-t+1}},
 \qquad c_0:=b,\quad c_n:=a,
\end{gathered}
\end{equation}
which is ordinary matrix multiplication
$(M^n)^a{}_b=M^a{}_{c_{n-1}}M^{c_{n-1}}{}_{c_{n-2}}\cdots M^{c_1}{}_b$
written out; for $n=1$ it reads $X^if_{ib}{}^a$, and for $n=2$ it
reproduces the third term of \eqref{eq:components} after renaming dummy
indices. The factorial series
