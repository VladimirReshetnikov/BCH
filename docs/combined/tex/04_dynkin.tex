...
letters may continue across a block boundary, for instance in the block word
$X\cdot XY$ followed by the block $Y$. Notice also that one must project a
\emph{whole homogeneous Lie polynomial}; replacing an arbitrary associative
polynomial by $D/n$ generally changes it. The right-nested brackets form a
...
$-\tfrac12(\tfrac12+\tfrac14+\tfrac12)=-\tfrac58$; the three three-piece cuts
...
hence in every associative specialization. The accompanying exact-arithmetic
program performs this comparison, including the zero coefficients, and
additionally recomputes both sides from the differential recursions of
\cref{sec:integral}.
\end{proof}
